...
class PlayState extends MusicBeatState
{
	...
	public static var isRobo:Bool = false;
	...
	override public function create()
	{
		...
		switch (curSong)
		{
			case 'Monochrome':
				...
				jumpScare = new FlxSprite().loadGraphic(Paths.image('scary/scuntscare', 'shared'));
		...
		if (generatedMusic)
			{
				...
				notes.forEachAlive(function(daNote:Note)
				{
					if (SONG.song == 'Infiltrator' && ghostNotes){	
						if (daNote.noteType == 0 && daNote.isOnScreen()) // if the note is on screen, it will lose transparency every frame 
						{
							daNote.alpha -= 0.025;
							//trace(daNote.alpha);
						}
	...
	*/
	...
	override function stepHit()
	{
		...
		if (curSong == 'Maggots')
		{
			if (soldierShits.contains(curStep) && !inCutscene) // lmao if the inCutscene is removed you can get killed in the dialogue which is funny -tob
	...
	function jumpscare(duration:Float, jumpScareAlpha:Float) 
		{
			...
			jumpScare.alpha = jumpScareAlpha;
